...
from ..crud import db, get_blindbit_config, get_silnt_wallet, get_silnt_wallets, get_wallet_addresses, insert_utxos_for_wallet, update_balance
...
class OwnedUTXO:
    ...
    def to_db_row(self, wallet_id: str) -> dict:
        return {"txid": self.txid.hex(), "vout": self.vout, "amount": self.amount,
                "priv_key_tweak": self.priv_key_tweak.hex(), "pub_key": self.pub_key.hex(),
                "utxo_state": self.utxo_state, "timestamp": self.timestamp, "wallet_id": wallet_id}


def request_scan_stop(wallet_id: str): _scan_stop[wallet_id] = True
def should_stop(wallet_id: str) -> bool: return _scan_stop.get(wallet_id, False)
def clear_scan_stop(wallet_id: str): _scan_stop.pop(wallet_id, None)
...
def create_shared_secret(public_component: bytes, scan_key: bytes, input_hash: Optional[bytes] = None) -> bytes:
    if input_hash is not None:
        scalar = (int.from_bytes(scan_key, "big") * int.from_bytes(input_hash, "big")) % SECP256K1_N
...
def create_output_pub_key_and_tweak(shared_secret: bytes, spend_pub_key: bytes, k: int) -> tuple[bytes, bytes]:
    t_k = _tagged_hash("BIP0352/SharedSecret", shared_secret + _ser_u32(k))
    p_k = PublicKey(spend_pub_key).combine([PublicKey.from_secret(t_k)]).format(compressed=True)
    return p_k[1:], t_k

def add_public_keys(pk1: bytes, pk2: bytes) -> bytes:
    if isinstance(pk1, PublicKey): pk1 = pk1.format(compressed=True)
    if isinstance(pk2, PublicKey): pk2 = pk2.format(compressed=True)
...
def add_private_keys(sk1: bytes, sk2: bytes) -> bytes:
    return ((int.from_bytes(sk1, "big") + int.from_bytes(sk2, "big")) % SECP256K1_N).to_bytes(32, "big")
...
def create_label(scan_key: bytes, m: int) -> Label:
    ...
    # ).digest()
    return Label(pub_key=PublicKey.from_secret(tweak).format(compressed=True), tweak=tweak, m=m)

def create_labels(scan_key: bytes, indices: list[int]) -> list[Label]:
    return [create_label(scan_key, m) for m in sorted(set([0] + indices))]

def match_labels(tx_output_33: bytes, pk_33: bytes, labels: list[Label]) -> Optional[Label]:
    try:
...
def receiver_scan_transaction_with_shared_secret(
    scan_key: bytes, spend_pub_key: bytes, labels: list[Label],
    tx_outputs: list[bytes], shared_secret: bytes,
) -> list[FoundOutput]:
    ...
    while True:
        ...
        for i, tx_output in enumerate(remaining):
            if output_pub_key == tx_output:
                ...
                remaining.pop(i); found = True; k += 1; break
            if not labels:
                continue
            ...
            if fl:
                found_outputs.append(FoundOutput(output=tx_output, sec_key_tweak=add_private_keys(tweak, fl.tweak), label=fl))
                remaining.pop(i); found = True; k += 1; break
            tx_out_neg = negate_public_key(tx_out_33)
            fl = match_labels(tx_out_neg, out_pk_33, labels)
            if fl:
                found_outputs.append(FoundOutput(output=tx_out_neg[1:], sec_key_tweak=add_private_keys(tweak, fl.tweak), label=fl))
                remaining.pop(i); found = True; k += 1; break
        if not found:
            break
    return found_outputs

def receiver_scan_transaction(scan_key, spend_pub_key, labels, tx_outputs, public_component, input_hash):
    return receiver_scan_transaction_with_shared_secret(
        scan_key, spend_pub_key, labels, tx_outputs,
...
def sync_block(tweaks, utxos, scan_key, spend_pub_key, labels):
    ...
    for tw, rel_utxos in to_check.items():
        found = receiver_scan_transaction(scan_key, spend_pub_key, labels,
            [bytes.fromhex(u["pubkey"]) for u in rel_utxos], tw, None)
        for fo in found:
            for u in rel_utxos:
                if fo.output == bytes.fromhex(u["pubkey"]):
                    owned.append(OwnedUTXO(txid=bytes.fromhex(u["txid"]), vout=u["vout"],
                        amount=u["amount"], priv_key_tweak=fo.sec_key_tweak, pub_key=fo.output,
                        utxo_state="unspent", timestamp=u.get("timestamp", 0), label=fo.label))
                    break
    return owned
        ...
        try:
            ...
            while True:
                ...
                if opk.hex()[:16] in shorts:
                    owned.append(OwnedUTXO(txid=bytes.fromhex(txid), vout=0, amount=0,
                        priv_key_tweak=t_k, pub_key=opk, utxo_state="unspent"))
                    matched = True
                if labels:
                    b33 = b"\x02" + opk
                    for label in labels:
                        try:
                            ...
                            if lo[1:].hex()[:16] in shorts:
                                owned.append(OwnedUTXO(txid=bytes.fromhex(txid), vout=0, amount=0,
                                    priv_key_tweak=add_private_keys(t_k, label.tweak),
                                    pub_key=lo[1:], utxo_state="unspent", label=label))
                                matched = True
                        except Exception:
                            pass
                if not matched:
...
async def scan_block(height, client, scan_secret_bytes, spend_pub_bytes, spend_secret_bytes, labels):
    if labels:
    ...
    if compute_data:
        matches = sync_block_from_compute_index(compute_data["index"], scan_secret_bytes, spend_pub_bytes, labels)
        if matches:
            ...
            for owned in matches:
                ...
                full = lkp.get(ph) or next((u for u in full_utxos if u.get("pubkey","")[:16]==ph[:16]), None)
                if full:
                    owned.vout = full.get("vout", owned.vout)
                    owned.amount = full.get("amount", owned.amount)
                    owned.timestamp = full.get("timestamp") or await get_block_ts(full.get("txid",""))
        ...
        try:
            ...
            for row in rows:
                ...
                if short_pub in spent_set:
                    ...
                    logger.info(f"Marked {row['txid']}:{row['vout']} spent at block {height}")
        except Exception as e:
...
async def set_last_scan_height(wallet_id: str, height: int) -> None:
    await db.execute("UPDATE silnt.wallets SET last_scan_height = :height WHERE id = :id",
                     {"height": height, "id": wallet_id})

def get_scan_progress(wallet_id: str) -> dict:
    return _scan_progress.get(wallet_id, {"active": False, "current": 0, "total": 0, "found": 0})

def set_scan_progress(wallet_id, current, total, found, active=True):
    _scan_progress[wallet_id] = {"active": active, "current": current, "total": total, "found": found}


async def scan_wallet(wallet_id: str, from_height: Optional[int] = None, to_height: Optional[int] = None) -> dict:
    ...
    scan_secret_hex = await decrypt_secret(wallet.scan_secret)
    scan_secret_bytes = bytes.fromhex(scan_secret_hex)
    row = await db.fetchone("SELECT spend_key FROM silnt.wallets WHERE id = :id", {"id": wallet_id})
    if not row or not row["spend_key"]:
        raise ValueError(f"No spend key for wallet {wallet_id}")
    spend_secret_bytes = bytes.fromhex(decrypt_spend_key(row["spend_key"], scan_secret_hex))
    ...
    labels = create_labels(scan_secret_bytes, indices=[a.label_index for a in saved_addresses])    
    ...
    for batch_start in range(0, total_blocks, BATCH_SIZE):
        ...
        batch = list(range(start + batch_start, min(start + batch_start + BATCH_SIZE, end + 1)))
        ...
        batch_results = await asyncio.gather(*[
            scan_block(h, oracle, scan_secret_bytes, spend_pub_bytes, spend_secret_bytes, labels)
            for h in batch], return_exceptions=True)

        await mark_spent_utxos_batch(batch, oracle, wallet_id, owned_utxos_lookup)

        for h, result in zip(batch, batch_results):
            if isinstance(result, Exception):
                logger.error(f"Block {h} error: {result}"); continue
            if result:
    ...
    logger.info(f"Scan done: {blocks_scanned} blocks, {total_found} UTXOs, balance={balance}")
    set_scan_progress(wallet_id, total_blocks, total_blocks, total_found, active=False)
    return {"utxos_found": total_found, "blocks_scanned": blocks_scanned,
            "final_height": last_scanned_height, "balance": balance}
