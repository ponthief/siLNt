...
async def _notify_payment_found(wallet, new_found: int, amount_sats) -> None:
    ...
    try:
        ...
        await send_fcm(
            tokens,
            "Payment received",
            "You've received a new payment. Open WhiSPa to view.",
            {"type": "payment"},
        )
    except Exception as e:
...
async def _notify_send_confirmed(user_id: str) -> None:
    ...
    try:
        ...
        await send_fcm(
            tokens,
            "Payment confirmed",
            "Your payment has its first confirmation. Open WhiSPa to view.",
            {"type": "send_confirmed"},
        )
    except Exception as e:
...
async def api_send_invite(
    ...
    """Invite a friend to WhiSPa by email.
    ...
    if not already:
        ...
        inviter_name = (inviter.username if inviter else None) or "A WhiSPa user"

        from .helpers.appenv import frontend_base_url

        origin = frontend_base_url(request)
        signup_url = f"{origin}/register"
        subject = f"{inviter_name} invited you to try WhiSPa"
        body = (
            f"Hi,\n\n"
            f"{inviter_name} is using WhiSPa — a Bitcoin wallet built on Silent "
...
# ── Silent Payments transaction helpers ──────────────────────────────────────
#
# Shared by Tango below, and written for the Silent Payments PayJoin that used
# to live here. That feature is gone — Tango replaced it in both clients — but
# these four are about coins and transactions rather than about who is paying
# whom, so they outlived it. helpers/payjoin_sp.py, which holds the
# cryptography, stays for the same reason.
#
# NO REQUEST BELOW CARRIES A KEY, and nothing below uses one:
#
...
#            scanning, and not read here: each party derives its own outputs at
#            home, so this works for a wallet that never uploaded one.
...
#            whose other half is b_spend, and it cannot sign alone. Nothing
#            here needs or reads it: _pj_payjoin_inputs deliberately leaves
#            PayjoinInput.priv_key_tweak unset, so the coordinator's copy of an
#            input could not sign for it even by accident.
...
async def _pj_broadcast(network: str, tx_hex: str) -> str:
    """Send a finished Tango to the network.
